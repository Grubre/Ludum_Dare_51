...
sf::Color ColorIDMap::generate_unique_color_id([[maybe_unused]]std::weak_ptr<Node> node)
{
    int blue  = 0;
    int green = 0;
    int red   = 0;
...
std::optional<sf::Color> ColorIDMap::get_color_at(engine::Vec2i at) {
    ...
    if(!((position.x < 0 || position.x >= static_cast<int>(image.getSize().x)) || (position.y < 0 || position.y >= static_cast<int>(image.getSize().y))) )
    {
